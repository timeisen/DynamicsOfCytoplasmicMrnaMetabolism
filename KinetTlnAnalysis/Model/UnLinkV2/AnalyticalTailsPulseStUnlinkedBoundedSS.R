...
offset=35
time_points<-(c(40,60,120,240,480)-offset)
...
Simulation<-function(pars,time){ ##cleanup
  ...
  tailsSS <- steady(func = "ode_deriv", y = tails[nrow(tails),-c(1,252)], parms = parameters, times=c(timeSS),method = "stode",bandup = 0, banddown = 1,nspec = max_tail*N, dllname = "AnalyticalTailsPulseStUnlinked",nout=1,initfunc = "ode_p_init")$y
  tails <- rbind(tails[-1,-c(1,253)],tailsSS)
  return(tails)
}
...
CalculateResidual <- function(pars,data,plot=FALSE,time_points) {
  ...
  if(is.null(tails) | any(is.na(tails)) | nrow(tails) != length(time_points)+1 | sum(tails) == 0) {
    print("max")
    return(10E20)}
  frac <- sum(tails[length(time_points)+1,231:251])/sum(tails[length(time_points)+1,3:251])
  if(is.na(frac)){
    print(tails)
    print(frac)
    }
  scaling = plogis(frac,location = 0.2,scale = .001)*10E10+1
  columns_to_remove = c(1,(250 - 6):(ncol(tails)))
  sim <- tails[,-columns_to_remove]
  model <- c(t(sim))
  sqs<-(model-data)^2
