...
rates <- fread("rate_constant_measurements/miR-155_minus_samples_UNLINKV76_run10_last7ntRm_Bounded_randomizedParam.txt")
rates <- as.numeric(rates[which(rates$V1 == accession),2:5])
if(any(is.na(rates))){
  break
}
rates[2] <- rates[2] * 0.1983597/0.264248
...
CalculateResidual <- function(pars,data,plot=FALSE,time_points,rates) {
  tails<-Simulation(2^pars,time_points,rates)
...
Optimization<-function(initial_param,data,plot=FALSE,time_points,rates){
  ...
  optim_param<-NULL
  for(x in 1:10){
    initial_param<-log2(runif(2,min=c(2.650033e-01,0.01*2.650033e-01),max=c(9.549402e-01,100*9.549402e-01)))
  ...
  optim_param <- data.frame(t(optim_param[which.min(optim_param[,3]),]))
  return(optim_param)
  }
