...
#This is the current model as of 2019 01 02. 
...
Simulation<-function(pars,time){ ##cleanup
  ...
  k        = pars[3]
  b        = pars[4]
  # size     = 11.566254 #12.66052 #These parameters are from fitting with the steady state tail seq data, plus. (hashed are minus)
  # location = 264.601362 #270.37422
  # scale    = 9.742162 #10.80678
  size     = 12.66052 #These parameters are from fitting with the steady state tail seq data. (minus)
  location = 270.37422
  scale    = 10.80678
...
CalculateResidual <- function(pars,data,plot=FALSE,time_points) {
  tails<-Simulation(2^pars,time_points)
  if(is.null(tails) | any(is.na(tails)) | nrow(tails) != length(time_points)+1 | sum(tails[,-1]) == 0) {
    print("max")
    return(10E20)}
  frac <- sum(tails[length(time_points)+1,232:252])/sum(tails[length(time_points)+1,3:252])
  if(is.na(frac)){
    print(tails)
    print(frac)
    }
  
  scaling = plogis(frac,location = 0.2,scale = .001)*10E10+1
  columns_to_remove = c(1,2,(251 - 6):ncol(tails))
  ...
  tick <<- tick + 1
  if(is.finite(residual)){return(residual*scaling)}
  else(return(10E20))
}
...
Optimization<-function(initial_param,data,plot=FALSE,time_points){
  ...
  optim_param<-NULL
  for(x in 1:10){
    initial_param<-log2(runif(4,min=c(147.16777,4.317860e-08,2.650033e-01,0.01*2.650033e-01),max=c(168.41418,1.396230e-07,9.549402e-01,100*9.549402e-01)))
  ...
  optim_param <- data.frame(t(optim_param[which.min(optim_param[,5]),]))
  return(optim_param)
  }
