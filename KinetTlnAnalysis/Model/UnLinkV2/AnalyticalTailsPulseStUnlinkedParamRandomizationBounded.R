...
Optimization<-function(initial_param,data,plot=FALSE,time_points){
  ...
  optim_param<-NULL
  for(x in 1:10){
    # initial_param<-log2(runif(3,min=c(1,10E-10,10E-10),max=c(250,10E10,10E10)))
    initial_param<-log2(runif(4,min=c(134.2714,2.366310e-08,0.1321663,0.03031561),max=c(188.2605,6.922078e-07,6.4182735,26.08015665))) #this is 5th to 95th percentile
  ...
  mmn <- apply(data.frame(optim_param),2,mean)
  msd <- apply(data.frame(optim_param),2,sd)
  optim_param <- c(mmn,msd)
  return(optim_param)
  }
...
all_optimizations<-cbind(accession,all_optimizations)
