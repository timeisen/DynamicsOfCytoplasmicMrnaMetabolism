...
# all_data_fn<-"/lab/solexa_bartel/teisen/Tail-seq/miR-155_final_analyses/model_input_files/randomized_gene_sets/miR-155_minus_rand_g_v6_last8rm_"
# i <- args[1]
...
all_data<-fread("model_input_files/TEST5_genes_miR-155_minus_sample_means_background_subtracted_v6_st_global_last8rm.txt",head=TRUE)
data<-as.matrix(all_data)
i<-"test"
print(detectCores())
#dataset params
...
N <<- ncol(data)/241 #number of genes
accession<<-colnames(all_data)[1+241*c(0:(N-1))]
initial_param<-c(rep(c(140,1E-7,1,1),N),7.2,241,10)
out_file <- paste0("/lab/solexa_bartel/teisen/Tail-seq/miR-155_final_analyses/rate_constant_measurements/global_model_fits/v7/Multicore_20180918_lim3_",i,".txt")

data_split <- split((1:ncol(data)), ceiling((1:ncol(data))/241))
...
Simulation<-function(pars,time){ ##cleanup
  ...
  columns_to_remove = c(1,2,(max_tail - 7):ncol(tails))
...
CalculateResidual <- function(pars,data,time_points) {
  ...
  residual <- .C("ReturnResidualRm8",n = as.integer(N),model = as.double(model),data = as.double(data),var = as.double(vars),residual = as.double(0))$residual
  # stopCluster(cl)
  # toc()
...
Optimization<-function(initial_param,data,plot=FALSE,time_points){
  ...
  for(x in 1:1){
    ...
    for(i in 1:3){
      ...
      ub=log2(c(rep(c(10E10,10E10,10E10,10E10),N),100,10000,10000)),
