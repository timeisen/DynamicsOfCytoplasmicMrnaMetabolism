...
# gcc -std=gnu99 -I/usr/share/R/include -DNDEBUG      -fpic -O3 -fstack-protector --param=ssp-buffer-size=4 -Wformat -Werror=format-security -D_FORTIFY_SOURCE=2  -c /lab/solexa_bartel/teisen/RNAseq/Scripts/models/compiled/R_helpers_V3.c -o /lab/solexa_bartel/teisen/RNAseq/Scripts/models/compiled/R_helpers_V3.o
# gcc -std=gnu99 -shared -L/usr/lib/R/lib -Wl,-Bsymbolic-functions -Wl,-z,relro -o /lab/solexa_bartel/teisen/RNAseq/Scripts/models/compiled/R_helpers_V3.so /lab/solexa_bartel/teisen/RNAseq/Scripts/models/compiled/R_helpers_V3.o -L/usr/lib/R/lib -lR
...
dyn.load("/lab/solexa_bartel/teisen/RNAseq/Scripts/models/compiled/R_helpers_V3.so") #load the model dynamically
...
all_data_fn<-"/lab/solexa_bartel/teisen/Tail-seq/miR-155_final_analyses/model_input_files/randomized_gene_sets/miR-155_minus_rand_g_v6_last8rm_SCALED_"
i <- args[1]
...
# all_data<-fread("model_input_files/TEST5_genes_miR-155_minus_sample_means_background_subtracted_v6_st_global_last8rm_SCALED.txt",head=TRUE)
# data<-as.matrix(all_data)
# i<-"test"
print(detectCores())
#dataset params
...
out_file <- paste0("/lab/solexa_bartel/teisen/Tail-seq/miR-155_final_analyses/rate_constant_measurements/global_model_fits/v7/Multicore_20180920_lim1_SCALED_run2_",i,".txt")

data_split <- split((1:ncol(data)), ceiling((1:ncol(data))/250))
data = lapply(data_split,function(index,data){data[,index]},data)
data = lapply(data,function(x){x = cbind(x[,1:241],rowSums(x[,242:250]))}) #now the data have 243 columns
data <- unlist(data)
...
Simulation<-function(pars,time){ ##cleanup
  ...
  sim <- cbind(sim[,1:241],rowSums(sim[,242:250])) ##This line sums the last 8 nt. 
  return(sim)
}
...
CalculateResidual <- function(pars,data,time_points) {
  ...
  residual <- .C("ReturnResidualModScale",n = as.integer(N),model = as.double(model),data = as.double(data),var = as.double(vars),residual = as.double(0))$residual
  # stopCluster(cl)
  # toc()
