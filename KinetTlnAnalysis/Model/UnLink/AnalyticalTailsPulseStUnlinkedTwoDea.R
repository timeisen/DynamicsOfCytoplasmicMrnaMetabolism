...
Simulation<-function(pars,time){ ##cleanup
  ...
  tails <- ode.band(func = "ode_deriv", y = initial_state, parms = parameters, times=c(0,time),method = "lsode",bandup = 0, banddown = 1,nspec = max_tail*N, dllname = "AnalyticalTailsPulseStUnlinkedGlobalTwoDea",nout=1,initfunc = "ode_p_init",hmax = 60)
...
Optimization<-function(initial_param,data,plot=FALSE,time_points){
  ...
  optim_param<-NULL
  for(x in 1:1){
    initial_param<-log2(runif(5,min=c(147.16777,4.317860e-08,2.650033e-01,2.650033e-01,100*2.650033e-01),max=c(168.41418,1.396230e-07,9.549402e-01,9.549402e-01,100*9.549402e-01)))
