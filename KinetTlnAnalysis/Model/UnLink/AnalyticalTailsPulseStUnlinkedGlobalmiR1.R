...
all_data_fn<-"/lab/solexa_bartel/teisen/Tail-seq/miR-155_final_analyses/model_input_files/randomized_gene_sets/miR-1_minus_rand_g_v6_"
i <- args[1]
...
out_file <- paste0("/lab/solexa_bartel/teisen/Tail-seq/miR-155_final_analyses/rate_constant_measurements/global_model_fits/v6/Multicore_20180703_miR1_",i,".txt")
...
CalculateResidual <- function(pars,data,time_points) {
  ...
  residual <- .C("ReturnResidualmiR1",n = as.integer(N),model = as.double(model),data = as.double(data),var = as.double(vars),residual = as.double(0))$residual
  # stopCluster(cl)
  # toc()
...
Optimization<-function(initial_param,data,plot=FALSE,time_points){
  ...
  for(x in 1:1){
    ...
    for(i in 1:3){
      ...
      data=data,
      # plot=FALSE,
      lb=log2(c(rep(c(1E-10,10E-10,10E-10,10E-10),N),0,0,0)),
