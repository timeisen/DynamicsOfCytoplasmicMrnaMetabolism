...
i <- args[1]
j <- args[2]
all_data<-fread(paste0(all_data_fn,i,"_BOOTSTRAP_",j,".txt"),head=TRUE)
...
initial_param<-c(rep(c(140,1E-7,1),N),1,7.2,250,10)
out_file <- paste0("rate_constant_measurements/global_model_fits/bootstrapOneDcp/","miR-155_minus_rand_g_rates_plogis_oneDcp_",i,"_BOOTSTRAP_",j,".txt")
...
CalculateResidual <- function(pars,data,time_points) {
  tick<<-tick+1
  param_list <- split(pars[1:(N*3)], ceiling(1:(N*3)/3))  
  model <- unlist(mclapply(param_list,function(gene_par,pars,time_points){Simulation(2^c(gene_par,tail(pars,4)),time_points)},pars,time_points))
...
Optimization<-function(initial_param,data,plot=FALSE,time_points){
  ...
  for(x in 1:1){
    ...
    for(i in 1:3){
      ...
      data=data,
      # plot=FALSE,
      lb=log2(c(rep(c(10E-10,10E-10,10E-10),N),10E-10,0,0,0)),
      ub=log2(c(rep(c(10E10,10E10,10E10),N),10E10,100,500,500)),
...
optim_data<-data.frame(matrix(all_optimizations[-c(lens,lens-1,lens-2,lens-3,lens-4)],ncol=3,byrow=TRUE))
colnames(optim_data)<-c("st","a","k")
optim_data$b <- all_optimizations[lens-4]
