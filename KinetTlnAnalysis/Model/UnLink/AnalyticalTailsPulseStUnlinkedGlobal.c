...
#include <R.h>
#include <Rmath.h>
// uses decapping with a logis function
static double p[4*5+3]; /* the number of parameters, genes*3 right not */
/* Function to initialize the parameters:.....................................*/
void ode_p_init(void (* odep)(int *, double *))
{
	int N=4*5+3	; /* number of parameters again */
	odep(&N, p);
}
	...
	double *yout, int *ip)
{
	...
	#define G   	   5  						/* Number of genes   		  */
	#define size       p[4*G+0]
	#define location   p[4*G+1]							/* tx size param for nbinom   */
	#define scale      p[4*G+2]							/* tx size param for nbinom   */
	...
	for (g = 0; g < G; g++) /* genes first	  */
	{
		...
		k_dea = p[4*g+2]; /* deadenylation rate */
		b     = p[4*g+3]; /* deadenylation rate */
