...
# See the miR-155 code for a description of the V4 and V5 script. 
...
Simulation <- function(pars,time){ ##This is the main simulator
  ...
  k        = pars[3]
  b        = pars[4]
  size     = 15.54483 #Global fitted parameters as of 2019 07 14, final. 
  location = 265.44891
  scale    = 13.97311
  ...
  last20ntMean = rep(mean(tails[6,(max_tail - 18):(ncol(tails)-1)]), 20)
...
CalculateResidual <- function(pars,data,plot=FALSE,time_points) { #Run the model
  ...
  sqs[1151:1170]  <- sqs[1151:1170] / vars[6]
