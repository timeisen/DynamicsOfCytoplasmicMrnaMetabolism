...
smooth <- function(x){
  ...
  smDat = c(
    ksmooth(8:249,a1,bandwidth = 10,kernel = "normal")$y,
    ksmooth(8:249,a2,bandwidth = 10,kernel = "normal")$y,
    ksmooth(8:249,a3,bandwidth = 10,kernel = "normal")$y,
    ksmooth(8:249,a4,bandwidth = 10,kernel = "normal")$y,
    ksmooth(8:249,a5,bandwidth = 10,kernel = "normal")$y,
    ksmooth(0:249,a6,bandwidth = 10,kernel = "normal")$y)  
}
...
Simulation <- function(pars,time){ ##This is the main simulator
  ...
  k        = pars[3]
  b        = pars[4]
  size     = 13.72803 #These params are from the prelim run, 2019 03 02, 22 datasets. 
  location = 274.12527
  scale    = 11.66877
  ...
  tails <- ode.band(func = "ode_deriv", y = initial_state, parms = parameters, 
          ...
          nout=1, initfunc = "ode_p_init", hmax = 60)
...
CalculateResidual <- function(pars,data,plot=FALSE,time_points) { #Run the model
  ...
  #Residuals, weighted.
  minDat = 7E-8 + 1E-8
  sqs <- (log(model+minDat)-log(data+minDat))^2
  residual <- sum(sqs)
  #For plotting
  if (tick%%10 == 0 & plot){
