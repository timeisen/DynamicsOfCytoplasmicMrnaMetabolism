...
Simulation <- function(pars,time){ ##This is the main simulator
  ...
  k        = pars[3]
  b        = pars[4]
  size     = 13.46757 #These parameters are from fitting with the steady state tail seq data. 
  location = 277.86290
  scale    = 13.97461
  ...
  last8nt = tails[6,(max_tail - 6):(ncol(tails)-1)]
...
CalculateResidual <- function(pars,data,plot=FALSE,time_points) { #Run the model
  ...
  sqs[969:1218]  <- sqs[969:1218] / vars[5]
  # sqs[1211:1460] <- sqs[1211:1460]/ vars[6] #for miR-155
