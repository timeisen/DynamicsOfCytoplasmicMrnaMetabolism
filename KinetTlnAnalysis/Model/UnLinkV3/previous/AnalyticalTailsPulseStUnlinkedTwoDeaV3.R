...
Simulation<-function(pars,time){ ##cleanup
  ...
  k2       = pars[4]
  b        = pars[5]
  size     = 20.409802   #110 #150: 13.671744   
  location = 254.330196  #110 #150: 266.342466 
  scale    = 8.799241    #110 #150: 9.676270   

  parameters = c(st,a,k1,k2,b,size,location,scale)
  max_tail = 251
  initial_state <- rep(0,max_tail*N)
  tails <- ode.band(func = "ode_deriv", y = initial_state, parms = parameters, times=c(0,time),method = "lsode",bandup = 0, banddown = 1,nspec = max_tail*N, dllname = "AnalyticalTailsPulseStUnlinkedGlobalTwoDea110",nout=1,initfunc = "ode_p_init",hmax = 1,maxsteps=5000000)
