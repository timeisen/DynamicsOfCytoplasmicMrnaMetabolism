...
args <- c("other_analyses/NMDsimulations/ModelSimDataV5b.txt","1485","v30")
#Some parameters for this gene are below:
########################################################################
# Min residual from two optimizations, previous run: 1.000166e+07
# True parameters, from simulation: 
# accession stGT alphaGT     ksGT     betaGT
#       1485  180   1e-07 7.943282 0.03162278
# Do I just not have the precision for this gene?
...
# The debug code uses a simulated gene that is not called correctly, from the 5b simulated data. 
...
plotData <- function(vec){
  plot( 249:0, vec[1211:1460],type = 'l',lwd = '2', log = 'y')
  lines(249:0, c(vec[1:242],rep(0,8)), lwd = '2', col = 'red')
  lines(249:0, c(vec[243:484],rep(0,8)), lwd = '2', col = 'orange')  
  lines(249:0, c(vec[485:726],rep(0,8)), lwd = '2', col = 'yellow')  
  lines(249:0, c(vec[727:968],rep(0,8)), lwd = '2', col = 'blue')  
  lines(249:0, c(vec[969:1210],rep(0,8)), lwd = '2', col = 'green') 
}

addDataToPlot <- function(vec){
  lines(249:0, vec[1211:1460],lty = 2)
  lines(249:0, c(vec[1:242],rep(0,8)),lty = 2,col = 'red')
  lines(249:0, c(vec[243:484],rep(0,8)),lty = 2,col = 'orange')  
  lines(249:0, c(vec[485:726],rep(0,8)),lty = 2,col = 'yellow')  
  lines(249:0, c(vec[727:968],rep(0,8)),lty = 2,col = 'blue')  
  lines(249:0, c(vec[969:1210],rep(0,8)),lty = 2,col = 'green') 
}
...
Simulation <- function(pars,time){ ##This is the main simulator
  ...
  k        = pars[3]
  b        = pars[4]
  size     = 14.38526 #These are globally fitted parameters 2019 04 22. 
  location = 264.00641
  scale    = 12.61589
...
CalculateResidual <- function(pars,data,plot=FALSE,time_points) { #Run the model
  ...
  #Residuals, weighted. 
  sqs <- (model-data)^2
  logres <- sum((log(model)-log(data))^2*1E6)
...
all_optimizations <- Optimization(
    log2(initial_param),
    data,plot=TRUE, time=time_points)
# print(accession)
