...
all_data_fn<-"/lab/solexa_bartel/teisen/Tail-seq/miR-155_final_analyses/model_input_files/randomized_gene_sets/miR-1_minus_rand_g_v7_last8rm_HYBRID20190731_"
i <- args[1]
...
out_file <- paste0("/lab/solexa_bartel/teisen/Tail-seq/miR-155_final_analyses/rate_constant_measurements/global_model_fits/v8/Multicore_HYBRID20190731_weighted_minus_miR-1_",i,".txt")
...
VecLast20 = (230+1):(250) 
for(i in 0:(N - 1)){
  data[5,VecLast20 + i * 250] = mean(data[5, VecLast20 + i * 250])
}
###############################################################
...
oneGeneRm = c((5*230+1):(5*250))
oneGeneAd = oneGeneRm[c((1:20)*5)]
oneGeneRm = oneGeneRm[-c((1:20)*5)]
...
Simulation<-function(pars,time){ ##cleanup
  ...
  sim[5,231:250] <- mean(sim[5,231:250]) #Averages the last 20 nt in last tp.
