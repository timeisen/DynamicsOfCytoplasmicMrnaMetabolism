...
# gcc -std=gnu99 -I/usr/share/R/include -DNDEBUG      -fpic -O3 -fstack-protector --param=ssp-buffer-size=4 -Wformat -Werror=format-security -D_FORTIFY_SOURCE=2  -c /lab/solexa_bartel/teisen/RNAseq/Scripts/models/compiled/R_helpers_V2.c -o /lab/solexa_bartel/teisen/RNAseq/Scripts/models/compiled/R_helpers_V2.o
# gcc -std=gnu99 -shared -L/usr/lib/R/lib -Wl,-Bsymbolic-functions -Wl,-z,relro -o /lab/solexa_bartel/teisen/RNAseq/Scripts/models/compiled/R_helpers_V2.so /lab/solexa_bartel/teisen/RNAseq/Scripts/models/compiled/R_helpers_V2.o -L/usr/lib/R/lib -lR
...
#V3 of the hybrid code was created on 2019 02 26 to work with the new direct ligation PAL-seq data. 
...
#The V4 version of the global hybrid code weights the last 8 nt of the steady state 6 fold higher.
#The V4_20 version of the global hybrid code uses datasets with that last 20 nt removed (through 19).

#read data in
#uncomment below to run
all_data_fn<-"/lab/solexa_bartel/teisen/Tail-seq/miR-155_final_analyses/model_input_files/randomized_gene_sets/miR-155_minus_rand_g_v7_last8rm_HYBRID20190731_" #Note that the global model datasets says last8rm but they are not. 
i <- args[1]
all_data<-fread(paste0(all_data_fn,i,".txt"),head=TRUE)
data<-as.matrix(all_data)

#uncomment below to test
# all_data<-fread("model_input_files/TEST5_genes_miR-155_minus_sample_means_background_subtracted_v7_st_global_last8rm_HYBRID.txt",head=TRUE)
# data<-as.matrix(all_data)
# i<-"test"
...
out_file <- paste0("/lab/solexa_bartel/teisen/Tail-seq/miR-155_final_analyses/rate_constant_measurements/global_model_fits/v8/Multicore_HYBRID20190731_weighted_minus_miR-155_",i,".txt")

###############################################################
##Averages last 20 nt of the steady state. 
##the earlier time points last 20 nt are excluded from the residual calculation.
VecLast20 = (230+1):(250) 
for(i in 0:(N - 1)){
  data[6,VecLast20 + i * 250] = mean(data[6, VecLast20 + i * 250])
}
###############################################################
...
##August 6 2019, updated direct lig data.
...
#scales that last 20 nt of the steady state by 6, for all of the time points.  
oneGeneRm = c((6*230+1):(6*250)) ## Modified by TJE 2019 07 10, next three lines.
oneGeneAd = oneGeneRm[c((1:20)*6)]
oneGeneRm = oneGeneRm[-c((1:20)*6)]

val0 <- unlist(lapply(oneGeneRm,function(x){x+(0:(N-1))*6*250}))
valM <- unlist(lapply(oneGeneAd,function(x){x+(0:(N-1))*6*250}))
vars[val0] = 0
vars[valM] = vars[valM] * 6
###############################################################
...
Simulation<-function(pars,time){ ##cleanup
  ...
  sim[6,231:250] <- mean(sim[6,231:250]) #Averages the last 20 nt in last tp.
