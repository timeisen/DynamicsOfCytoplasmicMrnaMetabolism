...
all_data_fn<-"/lab/solexa_bartel/teisen/Tail-seq/miR-155_final_analyses/model_input_files/randomized_gene_sets/miR-1_minus_rand_g_v7_last8rm_HYBRID20190603_"
i <- args[1]
...
out_file <- paste0("/lab/solexa_bartel/teisen/Tail-seq/miR-155_final_analyses/rate_constant_measurements/global_model_fits/v8/Multicore_HYBRID20190603_weighted_minus_miR-1_",i,".txt")
...
oneGeneAd = oneGeneRm[c((1:8)*5)]
oneGeneRm = oneGeneRm[-c((1:8)*5)]

val0 <- unlist(lapply(oneGeneRm,function(x){x+(0:(N-1))*5*250}))
valM <- unlist(lapply(oneGeneAd,function(x){x+(0:(N-1))*5*250}))
vars[val0] = 0
vars[valM] = vars[valM] * 5
###############################################################
