...
static double p[10]; /* the number of parameters, genes*3 right not */
/* Function to initialize the parameters:.....................................*/
void ode_p_init(void (* odep)(int *, double *))
{
	int N=10	; /* number of parameters again */
	odep(&N, p);
}
	...
	double *yout, int *ip)
{
	...
	#define locationmir	p[3*G+4]
	#define scalemir    p[3*G+5]
	#define bmir      	p[3*G+6]
	...
	for ( g = 0; g < G; g++) /* genes first	  */
	{
		...
		k_dec = plogis(0,location,scale,1,0)*b + plogis(T,locationmir,scalemir,1,0)*bmir; /* decay rate */
		ydot[g*T] = tx - (k_dea + k_dec)*y[g*T];
		for (l = 1; l < (T-1); l++)
		{
			tx   = dnbinom_mu(T - l, size, st_v, 0)*k_a;
			k_dec = plogis(l,location,scale,1,0)*b + plogis(T-l,locationmir,scalemir,1,0)*bmir; /* decay rate */

			ydot[l + g*T] = tx - (k_dea + k_dec)*y[l + g*T] + k_dea*y[l - 1 + g*T];
		}
		tx   = dnbinom_mu(1, size, st_v, 0)*k_a;
		k_dec  = plogis(T-1,location,scale,1,0)*b + plogis(1,locationmir,scalemir,1,0)*bmir;
