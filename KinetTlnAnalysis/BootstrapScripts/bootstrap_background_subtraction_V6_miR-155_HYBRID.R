...
det_SS_norm<-function(tp8hr,tpSS_,tp8hr_norm_counts,tpSS__norm_counts){
	hl<-read.table("/lab/solexa_bartel/eichhorn/5EU_RPF/miR-1_miR-155_analysis/Halflife_analysis/Halflife_measurements/miR-155_RNA_minusdox_alpha_hl_genenames.txt")
	hl<-hl[,c(2,3,4)]
	colnames(hl)<-c("accession","tx_rate","halflife")
	short_hl<-as.character(hl<-hl[which(hl$halflife<0.5),]$accession)
...
for(i in 1:10){
	...
	tpSWE<-read.table(paste0("/lab/solexa_bartel/teisen/Tail-seq/miR-155_final_analyses/single_tag_files/bootstrap/tail_lengths_annot_end_only_reformat_BOOTSTRAP_",i,".txt"),sep="\t")
	...
	print("Writing files ...")
	write.table(tp40m_bs,file=paste0("single_tag_files/background_subtracted_single_tag_files/bootstrap/tp40m_bs_minus_miR-155_v6_BOOTSTRAP_HYBRID20190326_",i,".txt"),quote=FALSE,row.names=FALSE,sep="\t",col.names=FALSE)
	write.table(tp1hr_bs,file=paste0("single_tag_files/background_subtracted_single_tag_files/bootstrap/tp1hr_bs_minus_miR-155_v6_BOOTSTRAP_HYBRID20190326_",i,".txt"),quote=FALSE,row.names=FALSE,sep="\t",col.names=FALSE)
	write.table(tp2hr_bs,file=paste0("single_tag_files/background_subtracted_single_tag_files/bootstrap/tp2hr_bs_minus_miR-155_v6_BOOTSTRAP_HYBRID20190326_",i,".txt"),quote=FALSE,row.names=FALSE,sep="\t",col.names=FALSE)
	write.table(tp4hr_bs,file=paste0("single_tag_files/background_subtracted_single_tag_files/bootstrap/tp4hr_bs_minus_miR-155_v6_BOOTSTRAP_HYBRID20190326_",i,".txt"),quote=FALSE,row.names=FALSE,sep="\t",col.names=FALSE)
	write.table(tp8hr_bs,file=paste0("single_tag_files/background_subtracted_single_tag_files/bootstrap/tp8hr_bs_minus_miR-155_v6_BOOTSTRAP_HYBRID20190326_",i,".txt"),quote=FALSE,row.names=FALSE,sep="\t",col.names=FALSE)
	write.table(tpSS__bs,file=paste0("single_tag_files/background_subtracted_single_tag_files/bootstrap/tpSS_minus_norm_miR-155_BOOTSTRAP_HYBRID20190326_",i,".txt"),quote=FALSE,row.names=FALSE,sep="\t",col.names=FALSE)
