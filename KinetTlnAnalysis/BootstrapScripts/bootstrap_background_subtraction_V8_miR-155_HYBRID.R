...
ShortTailNorm <- function(rowX, WindowSize){
	adjustment = sum(rowX[(252 + CUTOFF - WindowSize):(252 + CUTOFF + WindowSize)])/sum(rowX[(CUTOFF - WindowSize):(CUTOFF + WindowSize)])
...
for(i in 1:10){
	...
	## These lines perform the hrbid dataset conversion, which has been moved to a separate script. 20190805
	# DIR <- tpSWE
 #	SPL <- tpSS__bs
 ...
 #	SPL <- SPL[,c(252,1:251)] #make accession the first column
	...
	# SPLDIR <- SPLDIR[complete.cases(SPLDIR),]
	# print(dim(SPLDIR))
