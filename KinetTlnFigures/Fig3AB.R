#This code is for histogram of the half lives, for fig 3A
#This code is for violin plots of the half lives, for fig 3B
...
p1<-ggplot(halflife,aes(x=classification,y=halflife))+geom_violin(color="black")+theme_tim()+scale_y_continuous(trans="log10",breaks=log_ticks(.1,100)[[2]],labels=log_ticks(.1,100)[[1]],limits=c(0.1,101),name=NULL)+scale_x_discrete(name=NULL)
...
#histogram
p2 <- ggplot(halflife,aes(x=halflife,y=..density../50))+stat_bin(geom="step",bins=50,position='identity',color="black")+theme_tim()+scale_x_continuous(trans="log10",breaks=log_ticks(.1,100)[[2]],labels=log_ticks(.1,100)[[1]],limits=c(0.09,100),name=NULL)+scale_y_continuous(name=NULL,expand=c(0,0))
