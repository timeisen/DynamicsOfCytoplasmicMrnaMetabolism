...
a <- list(read.table("/lab/solexa_bartel/teisen/Tail-seq/miR-155_final_analyses/single_tag_files/background_subtracted_single_tag_files/SS_minus_norm_miR-1_v7_HYBRID20190326.txt"),read.table("/lab/solexa_bartel/teisen/Tail-seq/miR-155_final_analyses/single_tag_files/background_subtracted_single_tag_files/SS_minus_norm_miR-155_v7_HYBRID20190326.txt"))
dat = NULL
...
for(i in c(1:2)){
	...
	p1 <- ggplot(miR1_melt,aes(x=tail_length,y=abundance,color = bin))+
		geom_line() + 
		theme_bw() +
		scale_x_continuous(limits = c(1,249)) +
